...
- Fixed GUI issue for CAT offers where the CAT Tail would not show in the tooltip for `Unknown CAT`s (https://github.com/Chia-Network/tree-blockchain-gui/issues/950)
...
- The GUI is now in a separate submodule repository from chia-blockchain, [tree-blockchain-gui](https://github.com/Chia-Network/tree-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `tree-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
