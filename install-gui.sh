...
if [ "$(id -u)" = 0 ]; then
  echo "The Tree Blockchain GUI can not be installed or run by the root user."
  exit 1
fi
...
# https://github.com/Tree111-Network/tree-blockchain/pull/10460#issuecomment-1054492495
...
if [ ! "$CI" ]; then
  ...
  # Set modified output of `tree version` to version property of GUI's package.json
...
echo ""
echo "Tree blockchain install-gui.sh completed."
echo ""
