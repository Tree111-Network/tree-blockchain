...
from tree.simulator.start_simulator import async_main as start_simulator_main
from tree.ssl.create_ssl import create_all_ssl
from tree.types.blockchain_format.sized_bytes import bytes32
from tree.util.bech32m import encode_puzzle_hash
from tree.util.config import create_default_tree_config, load_config, save_config
from tree.util.errors import KeychainFingerprintExists
...
def create_config(
    ...
    create_default_tree_config(tree_root)
    create_all_ssl(
        tree_root,
    ...
    # load config
    config = load_config(tree_root, "config.yaml")
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(keychain, fingerprint), "ttree111")
    config["simulator"]["plot_directory"] = "test-simulator/plots"
    # save config
    save_config(tree_root, "config.yaml", config)
    return config


async def start_simulator(tree_root: Path, automated_testing: bool = False) -> AsyncGenerator[FullNodeSimulator, None]:
    sys.argv = [sys.argv[0]]  # clear sys.argv to avoid issues with config.yaml
    service = await start_simulator_main(True, automated_testing, root_path=tree_root)
    await service.start()
...
async def get_full_tree_simulator(
    tree_root: Path,
    ...
    """
    A tree root Path is required.
    The tree root Path can be a temporary directory (tempfile.TemporaryDirectory)
    ...
    This test is fully interdependent and can be used without the rest of the tree test suite.
    ...
    with Lockfile.create(daemon_launch_lock_path(tree_root)):
        ...
        crt_path = tree_root / config["daemon_ssl"]["private_crt"]
        key_path = tree_root / config["daemon_ssl"]["private_key"]
        ca_crt_path = tree_root / config["private_ssl_ca"]["crt"]
        ca_key_path = tree_root / config["private_ssl_ca"]["key"]

        shutdown_event = asyncio.Event()
        ws_server = WebSocketServer(tree_root, ca_crt_path, ca_key_path, crt_path, key_path, shutdown_event)
        await ws_server.setup_process_global_state()
        await ws_server.start()

        async for simulator in start_simulator(tree_root, automated_testing):
            yield simulator, tree_root, config, mnemonic, fingerprint, keychain
