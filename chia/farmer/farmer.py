...
from tree.plot_sync.delta import Delta
from tree.plot_sync.receiver import Receiver
from tree.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
...
from tree.util.bech32m import decode_puzzle_hash
from tree.util.byte_types import hexstr_to_bytes
from tree.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
from tree.util.errors import KeychainProxyConnectionFailure
from tree.util.hash import std_hash
from tree.util.ints import uint8, uint16, uint32, uint64
...
from tree.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'tree keys generate' or open the UI."
        try:
        ...
        if "tree111_target_address" not in self.config:
            self.config = config["farmer"]
        if "tree111_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "tree111_target_address" not in self.config or "tree111_target_address" not in self.pool_config:
            log.debug("tree111_target_address missing in the config")
            return False

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["tree111_target_address"]
        self.farmer_target = decode_puzzle_hash(self.farmer_target_encoded)

        self.pool_public_keys = [G1Element.from_bytes(bytes.fromhex(pk)) for pk in self.config["pool_public_keys"]]

        # This is the self pooling configuration, which is only used for original self-pooled plots
        self.pool_target_encoded = self.pool_config["tree111_target_address"]
    ...
    def on_disconnect(self, connection: WSTreeConnection):
    ...
    def set_reward_targets(self, farmer_target_encoded: Optional[str], pool_target_encoded: Optional[str]):
        with lock_and_load_config(self._root_path, "config.yaml") as config:
            if farmer_target_encoded is not None:
                ...
                config["farmer"]["tree111_target_address"] = farmer_target_encoded
            if pool_target_encoded is not None:
                self.pool_target_encoded = pool_target_encoded
                self.pool_target = decode_puzzle_hash(pool_target_encoded)
                config["pool"]["tree111_target_address"] = pool_target_encoded
