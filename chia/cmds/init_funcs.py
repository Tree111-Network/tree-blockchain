...
from tree.cmds.configure import configure
from tree.consensus.coinbase import create_puzzlehash_for_pk
from tree.ssl.create_ssl import create_all_ssl
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'tree keys generate'")
        return None

    with lock_and_load_config(new_root, "config.yaml") as config:
        pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
        all_targets = []
        stop_searching_for_farmer = "tree111_target_address" not in config["farmer"]
        stop_searching_for_pool = "tree111_target_address" not in config["pool"]
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("tree111_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("tree111_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("tree111_target_address") or all_targets[-2] == config["pool"].get(
                    "tree111_target_address"
                ):
                    stop_searching_for_pool = True

        # Set the destinations, if necessary
        updated_target: bool = False
        if "tree111_target_address" not in config["farmer"]:
            print(
                f"Setting the tree111 destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
            config["farmer"]["tree111_target_address"] = all_targets[0]
            updated_target = True
        elif config["farmer"]["tree111_target_address"] not in all_targets:
            print(
                f"WARNING: using a farmer address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['farmer']['tree111_target_address']} with {all_targets[0]}"
            )

        if "pool" not in config:
            config["pool"] = {}
        if "tree111_target_address" not in config["pool"]:
            print(f"Setting the tree111 destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
            config["pool"]["tree111_target_address"] = all_targets[0]
            updated_target = True
        elif config["pool"]["tree111_target_address"] not in all_targets:
            print(
                f"WARNING: using a pool address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['pool']['tree111_target_address']} with {all_targets[0]}"
            )
        if updated_target:
            print(
                f"To change the TREE111 destination addresses, edit the `tree111_target_address` entries in"
...
def init(
    ...
    if create_certs is not None:
        ...
        return tree_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)


def tree_version_number() -> Tuple[str, str, str, str]:
...
def tree_full_version_str() -> str:
    major, minor, patch, dev = tree_version_number()
...
def tree_init(
    ...
    *,
    ...
    """
    tree_root = os.environ.get("TREE_ROOT", None)
    if tree_root is not None:
        print(f"TREE_ROOT is set to {tree_root}")

    print(f"Tree directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if TREE_ROOT is set, or if user has run tree init twice
        # before a new update.
        if testnet:
            configure(
                root_path,
    ...
    print("")
    print("To see your keys, run 'tree keys show --show-mnemonic-seed'")
