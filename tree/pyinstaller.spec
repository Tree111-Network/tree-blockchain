...
ROOT = pathlib.Path(importlib.import_module("tree").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `tree` file name (which is the executable)
    and the `tree` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `tree` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("tree-blockchain"))[0]
...
# TODO: collapse all these entry points into one `tree_exec` entrypoint that accepts the server as a parameter

entry_points = ["tree.cmds.tree"] + [f"tree.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "tree.util.bip39"])
...
datas.append((f"{ROOT}/tree/util/english.txt", "tree/util"))
datas.append((f"{ROOT}/tree/util/initial-config.yaml", "tree/util"))
datas.append((f"{ROOT}/tree/wallet/puzzles/*.hex", "tree/wallet/puzzles"))
datas.append((f"{ROOT}/tree/ssl/*", "tree/ssl"))
...
add_binary("tree", f"{ROOT}/tree/cmds/tree.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/tree/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/tree/server/start_{server}.py", COLLECT_ARGS)

add_binary("start_crawler", f"{ROOT}/tree/seeder/start_crawler.py", COLLECT_ARGS)
add_binary("start_seeder", f"{ROOT}/tree/seeder/dns_server.py", COLLECT_ARGS)
add_binary("start_data_layer_http", f"{ROOT}/tree/data_layer/data_layer_server.py", COLLECT_ARGS)
