...
def validate_fee(ctx, param, value):
    try:
        ...
        raise click.BadParameter("Fee must be decimal dotted value in TREE111 (e.g. 0.05)")
    if fee < 0.05 or fee > MAX_CMDLINE_FEE:
        raise click.BadParameter(f"Fee must be in the range 0.05 to {MAX_CMDLINE_FEE}")
    return value
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in TREE111. Fee is used TWICE: once to create the singleton, once for init.",
    type=str,
    default="0",
    show_default=True,
    required=True,
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in TREE111. Fee is used TWICE: once to leave pool, once to join.",
    type=str,
    default="0",
    show_default=True,
    required=True,
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in TREE111. Fee is charged TWICE.",
    type=str,
    default="0",
    show_default=True,
    required=True,
