...
def show_plots(root_path: Path):
    from tree.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'tree plots add -d [dir]' and remove with"
        + " 'tree plots remove -d [dir]'"
        + " Scan and check plots with 'tree plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'tree init'")
...
def check_cmd(
    ...
    from tree.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from tree.plotting.util import remove_plot_directory
