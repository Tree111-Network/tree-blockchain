...
from tree.consensus.blockchain import ReceiveBlockResult
from tree.consensus.constants import ConsensusConstants
from tree.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate, wrap_local_keychain
from tree.protocols import wallet_protocol
from tree.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from tree.protocols.protocol_message_types import ProtocolMessageTypes
from tree.protocols.wallet_protocol import CoinState, RespondBlockHeader, RespondToCoinUpdates, RespondToPhUpdates
from tree.rpc.rpc_server import default_get_connections
from tree.server.node_discovery import WalletPeers
...
from tree.types.weight_proof import WeightProof
from tree.util.chunks import chunks
from tree.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED, process_config_start_method
from tree.util.errors import KeychainIsEmpty, KeychainIsLocked, KeychainKeyNotFound, KeychainProxyConnectionFailure
...
from tree.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from tree.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
from tree.wallet.util.wallet_sync_utils import (
...
from tree.wallet.wallet_weight_proof_handler import WalletWeightProofHandler, get_wp_fork_point
...
class WalletNode:
    ...
    _server: Optional[TreeServer] = None
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'tree keys' program.")
            return None
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.tree111.com"]),
    ...
    def on_disconnect(self, peer: WSTreeConnection):
    ...
    async def on_connect(self, peer: WSTreeConnection):
        if self._wallet_state_manager is None:
            return None
    ...
    async def is_peer_synced(
        self, peer: WSTreeConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSTreeConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSTreeConnection]:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSTreeConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            return None
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSTreeConnection]:
        """
        ...
        synced_and_trusted: List[WSTreeConnection] = []
        synced: List[WSTreeConnection] = []
        trusted: List[WSTreeConnection] = []
        neither: List[WSTreeConnection] = []
        all_nodes: List[WSTreeConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peers: List[WSTreeConnection] = self.get_full_node_peers_in_order()
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSTreeConnection):
        if self._wallet_state_manager is None:
    ...
    async def new_peak_from_trusted(self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSTreeConnection):
    ...
    async def new_peak_from_untrusted(
        self, new_peak_hb: HeaderBlock, peer: WSTreeConnection, request_time: uint64
    ) -> bool:
        far_behind: bool = (
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSTreeConnection):
        current_height: uint32 = await self.wallet_state_manager.blockchain.get_finished_sync_up_to()
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSTreeConnection) -> int:
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSTreeConnection, peak: HeaderBlock
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSTreeConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def fetch_puzzle_solution(self, height: uint32, coin: Coin, peer: WSTreeConnection) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSTreeConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSTreeConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
