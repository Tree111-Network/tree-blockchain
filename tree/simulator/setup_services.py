...
async def setup_wallet_node(
    ...
    tree111_spam_amount: int = 1000000,
    ...
    with TempKeyring(populate=True) as keychain:
        ...
        service_config["tree111_spam_amount"] = tree111_spam_amount
...
async def setup_farmer(
    ...
    service_config["tree111_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "tree111")
    service_config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    service_config["port"] = port
    service_config["rpc_port"] = uint16(0)
    config_pool["tree111_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "tree111")
