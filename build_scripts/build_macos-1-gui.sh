...
echo "Remove unused @tree packages to make cache slim"
ls -l packages
rm -rf packages/api
...
# Remove `packages/gui/node_modules/@tree` because it causes an error on later `electron-packager` command
rm -rf "@tree"
