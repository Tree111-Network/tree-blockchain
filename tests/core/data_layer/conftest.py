...
from tree.data_layer.data_layer_util import NodeType, Status
from tree.data_layer.data_store import DataStore
from tree.types.blockchain_format.tree_hash import bytes32
...
@pytest.fixture(name="tree_daemon", scope="function")
def tree_daemon_fixture(tree_root: TreeRoot) -> Iterator[None]:
    with closing_tree_root_popen(tree_root=tree_root, args=[sys.executable, "-m", "tree.daemon.server"]):
...
@pytest.fixture(name="tree_data", scope="function")
def tree_data_fixture(tree_root: TreeRoot, tree_daemon: None, scripts_path: pathlib.Path) -> Iterator[None]:
    with closing_tree_root_popen(tree_root=tree_root, args=[os.fspath(scripts_path.joinpath("tree_data_layer"))]):
        # TODO: this is not pretty as a hard coded time
        # let it settle
        time.sleep(5)
        yield
