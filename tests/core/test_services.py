...
from tree.util.misc import sendable_termination_signals
from tests.core.data_layer.util import TreeRoot
from tests.util.misc import closing_tree_root_popen
...
@pytest.mark.asyncio
async def test_daemon_terminates(signal_number: signal.Signals, tree_root: TreeRoot) -> None:
    port = find_available_listen_port()
    with lock_and_load_config(root_path=tree_root.path, filename="config.yaml") as config:
        config["daemon_port"] = port
        save_config(root_path=tree_root.path, filename="config.yaml", config_data=config)

    with closing_tree_root_popen(tree_root=tree_root, args=[sys.executable, "-m", "tree.daemon.server"]) as process:
        client = await wait_for_daemon_connection(root_path=tree_root.path, config=config)
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        [DataLayerRpcClient.create, "tree.server.start_data_layer", "data_layer"],
        [FarmerRpcClient.create, "tree.server.start_farmer", "farmer"],
        [FullNodeRpcClient.create, "tree.server.start_full_node", "full_node"],
        [HarvesterRpcClient.create, "tree.server.start_harvester", "harvester"],
        [WalletRpcClient.create, "tree.server.start_wallet", "wallet"],
        # TODO: review and somehow test the other services too
        # [, "tree.server.start_introducer", "introducer"],
        # [, "tree.seeder.start_crawler", ""],
        # [, "tree.server.start_timelord", "timelord"],
        # [, "tree.timelord.timelord_launcher", ],
        # [, "tree.simulator.start_simulator", ],
        # [, "tree.data_layer.data_layer_server", "data_layer"],
    ],
)
@pytest.mark.asyncio
async def test_services_terminate(
    ...
    with closing_tree_root_popen(
        tree_root=tree_root,
        args=[sys.executable, "-m", "tree.daemon.server"],
    ):
        ...
        # farmer.
        daemon_client = await wait_for_daemon_connection(root_path=tree_root.path, config=config)
