...
from tree.protocols import farmer_protocol
from tree.protocols.harvester_protocol import Plot
from tree.rpc.farmer_rpc_api import (
    FilterItem,
...
from tree.simulator.block_tools import get_plot_dir
...
async def test_farmer_reward_target_endpoints(harvester_farmer_environment):
    ...
    await farmer_rpc_client.set_reward_targets(encode_puzzle_hash(new_ph, "tree111"), encode_puzzle_hash(new_ph_2, "tree111"))
    ...
    await farmer_rpc_client.set_reward_targets(
        encode_puzzle_hash(observer_farmer, "tree111"), encode_puzzle_hash(observer_pool, "tree111")
    )
    ...
    assert config["farmer"]["tree111_target_address"] == encode_puzzle_hash(observer_farmer, "tree111")
    assert config["pool"]["tree111_target_address"] == encode_puzzle_hash(observer_pool, "tree111")

    new_ph_2_encoded = encode_puzzle_hash(new_ph_2, "tree111")
