...
from tree.simulator.simulator_full_node_rpc_client import SimulatorFullNodeRpcClient
from tree.simulator.simulator_test_tools import get_full_tree_simulator, get_puzzle_hash_from_key
...
class TestStartSimulator:
    ...
    async def get_tree_simulator(
        self, tmp_path: Path, empty_keyring: Keychain
    ) -> AsyncGenerator[Tuple[FullNodeSimulator, Path, Dict[str, Any], str, int, Keychain], None]:
        async for simulator_args in get_full_tree_simulator(tree_root=tmp_path, keychain=empty_keyring):
            yield simulator_args

    @pytest.mark.asyncio
    async def test_start_simulator(
        self, get_tree_simulator: Tuple[FullNodeSimulator, Path, Dict[str, Any], str, int, Keychain]
    ) -> None:
        simulator, root_path, config, mnemonic, fingerprint, keychain = get_tree_simulator
